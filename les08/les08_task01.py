...
class Game:
    ...
    def move(self):
        ...
        while len(self.move) > 0:
            ...
            user_ans = input(f'Выпало число: {self.move[del_number]} (Осталось {len(self.move)-1})\nЗачеркнуть цифру? y/n ')
            for i in range(0, 3):
                try:
                    ...
                    del_player_number = -1
                try:
                    del_pc_number = self.p2[i].index(self.move[del_number])
                    self.p2[i][del_pc_number] = '--'
                    self.p2.fine += 1
                    break
                except ValueError:
                    pass
            del self.move[del_number]
            if self.p1.fine == 15:
                print('Игрок победил')
                exit()
            if self.p2.fine == 15:
